   ...
   jQuery(".index-page__story-list .filler").append(text);
        ...
        function getUnitStories(number, start, render){
            ...
            returnedStories = jQuery.ajax({
                method: 'POST',
                url: 'https://api.thetyee.ca/v1/keyword/' + topic + '/' + storiesRequested + '/' + start,
                dataType: 'jsonp',
                data: response,
                crossDomain: true,
                ...
                error: function(){
                    console.log("error: " + 'https://api.thetyee.ca/v1/keyword/' + topic + '/' + storiesRequested + '/0' + response);
                }
            });
